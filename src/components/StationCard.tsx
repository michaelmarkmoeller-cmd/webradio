      ...
      >
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="flex-1 min-w-0">
            ...
            <span className="text-xs text-text-secondary mt-0.5 block">{station.category}</span>
          </div>
        ...
        <div className="flex items-center justify-between">
          ...
          >
            {isCurrentlyPlaying ? (
              ...
              <svg
                className={`w-4 h-4 ${isActive ? 'text-bg-primary' : 'text-text-secondary group-hover:text-accent'}`}
                fill="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          {isCurrentlyPlaying && (
            <div className="flex items-end gap-0.5 h-5">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="w-1 bg-accent rounded-sm animate-pulse"
                  style={{
                    height: `${40 + i * 15}%`,
