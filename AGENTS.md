...
**Lydløs pause på iOS (23-09-2026, BUG-15 genåbnet)**: Pause på iOS stopper **ikke** streamen — `pauseAudio(isIOS)` sætter `a.muted = true`, og streamen kører videre. UI og MediaSession viser pause; PLAY (app, låseskærm, AirPods) slår blot lyden til igen uden ny forbindelse. Formål: stoppes streamen, lægger iOS siden i dvale 10-15 sek. efter skærmen slukker, og et PLAY fra låseskærmen kan så ikke nå at koble på igen (widget'en forsvinder). Stoppes rigtigt efter `SILENT_PAUSE_VISIBLE_MS` (20 sek.) mens appen er synlig, ellers efter `SILENT_PAUSE_MAX_MS` (5 min.) fra pausen — `armSilentPauseTimer()` genberegnes ved hvert `visibilitychange`. `endSilentPause()` = rigtigt stop. `stopPlayback()` (søvntimer, Sonos-cast) stopper altid rigtigt. `play`-lytteren og `visibilitychange`-reconcileren ignorerer det lydløst kørende element, så UI ikke viser "spiller". Ren stilhed (ingen tone som den fjernede keepalive), men koster data/strøm i højst 5 min. Pc uændret. Afventer bekræftelse på iPhone (TC-17-16..18).
...
**BUG-15 genåbnet 23-09-2026:** lydløs pause på iOS deployet (commit `06accb1`) — afventer Michaels iPhone-test (TC-17-16..18).
...
- `tests/tc-17b.spec.ts` — TC-17-05..15: lydløs pause på iOS (11 tests; iPhone-UA, `page.clock`, simuleret `visibilityState`, MediaSession-handlere kaldes direkte). `WEBRADIO_URL` virker også her
