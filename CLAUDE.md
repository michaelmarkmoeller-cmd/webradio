...
**AirPods/headset og lydløs pause (løst 24-09-2026, iPhone 17 Pro Max, iOS 27.0, fundet via en midlertidig diagnose-log — nu fjernet)**: Ved AirPods ud sender iOS `ms:pause` → appen starter stilhedsløkken → **iOS stopper selv løkken 0,1-1 sek. senere** (enten `play()` afvist med `AbortError` eller en `pause`-hændelse efter start) — Apples regel om, at lyd ikke må fortsætte i højttaleren. `onLoopKilled()` genstarter løkken **én gang** efter 1,5 sek. (lykkes både synligt og med låst skærm); pause-hændelse + afvist `play()` fra samme stop samles via `silentPause.retry` (`'pending'`/`'done'`). `pause`-lytteren ved at det er iOS, fordi vores egne pauser af løkken altid rydder `silentPause` først. Mens løkken kører, ser iOS appen som *spillende*, så **headsettets afspil/pause-knap sender `ms:pause`** — den tolkes som PLAY under lydløs pause, undtagen de første 3 sek. efter pausen (iOS sender selv en ekstra `ms:pause` 0,5-1 sek. efter AirPods ud). **Accepterede begrænsninger:** ingen automatisk genstart ved AirPods ind (iOS sender ingen play), AirPods' "ding" udebliver mens løkken kører, og løkken kører uhørligt i højttaleren i højst 30 min. **Fravalgt:** at starte løkken når iOS pauser streamen *uden* `ms:pause` (kan være Spotify/opkald — ville stjæle lyden). **Fejlet forsøg:** et ekstra "bridge"-audio-element med stilhedsløkken (låst op lydløst i første klik) overtog iOS' Now Playing-tilknytning — widget'en forsvandt/viste forkert status. Brug aldrig mere end ét audio-element på iOS. **`devicechange` fyrer slet ikke på iOS** — `App.tsx`'s frakoblings-handler og `pauseForDisconnect()` virker kun på andre platforme (beholdt).

**iOS audio session keepalive — fjernet 14-07-2026**: Der var tidligere et separat `<audio>`-element (`src/audio.ts`, `startKeepalive()`/`stopKeepalive()`) der loopede en næsten-lydløs 18 Hz-tone for at forhindre iOS i at deaktivere audio-sessionen når streamen var pauset (så WebRadio blev stående som "Now Playing" på låseskærmen). Fjernet efter Michaels beslutning 14-07-2026: tonen kunne blive hørbar som brummen på tilsluttede subwoofere (fx Sonos hjemmebiograf-grupper), og holdt desuden telefonen unødigt aktiv/strømforbrugende selv under en pause. **Kendt konsekvens:** WebRadio kan nu forsvinde fra låseskærmen, efter streamen har været pauset et stykke tid og telefonen baggrundslægges — dette var en bevidst accepteret platformsbegrænsning — **afløst 23-24-09-2026 af lydløs pause** (stilhedsløkke uden tone), se ovenfor. Se `BUGS.md` (BUG-14) for hele forløbet (tidsbegrænset variant blev forsøgt først, derefter droppet helt).
