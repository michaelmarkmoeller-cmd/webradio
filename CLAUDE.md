...
**PLAY på låst skærm efter pause kan fejle stille (BUG-15, accepteret platformsbegrænsning 14-07-2026 — genåbnet 23-09-2026, forsøgt løst med lydløs pause ovenfor)**: Trykker man PLAY på låseskærmens medie-widget, mens telefonen er låst/baggrundslagt, strupper iOS baggrunds-netværksadgangen ofte så hårdt, at `a.play()`s promise hverken opfyldes eller afvises — efter ca. 5 sek. viser iOS selv "Afspiller ikke" og fjerner widget'en helt, uden at WebRadios egen fejl-håndtering nogensinde nåede at reagere. Går man derefter selv ind i appen, opdager `visibilitychange`-reconcileren uoverensstemmelsen og arm'er `_shouldResume` (se nedenfor) — næste tryk hvor som helst i appen genoptager typisk automatisk. Samme mønster kan opstå ved almindeligt app-skift/Safari-navigation (ikke kun låseskærm). Se `BUGS.md` (BUG-15) for fuld reproduktion og analyse — kun symptomet "hakkende lyd ved efterfølgende manuelt gentryk" er rettet (se ovenfor), selve "ingen lyd første gang" er accepteret, samme grundårsag som BUG-14.

**Pause fade-out**: `togglePlay()` fader volume til 0 over 80ms (8 trin × 10ms) inden `audio.pause()` — eliminerer det waveform-klik der opstår ved abrupt afskæring (kun pc — på iOS er volume read-only, se lydløs pause).

**Lydløs pause på iOS (23-09-2026, BUG-15 genåbnet)**: Pause på iOS stopper **ikke** afspilningen — `pauseAudio(isIOS)` skifter audio-elementet til en lokal stilhedsløkke (`getSilentLoopUrl()` i `audio.ts`: 2 sek. WAV-blob med ±1 LSB støj ≈ −90 dBFS, `loop = true`). iOS ser afspilning og holder siden vågen og WebRadio på låseskærmen; UI og MediaSession viser pause. PLAY (app, låseskærm, headset-knap) kobler radiostreamen på igen — virker, fordi siden stadig er vågen. Intet dataforbrug under pausen. Formål: stoppes afspilningen, lægger iOS siden i dvale 10-15 sek. efter skærmen slukker, og et PLAY fra låseskærmen kan så ikke nå at koble på igen (widget'en forsvinder). **`a.muted = true` virker IKKE** (første forsøg, commit `06accb1`): iOS behandlede den slået-fra stream som stoppet — WebRadio forsvandt fra låseskærmen med det samme, og Spotifys gamle cover blev vist kort. Stoppes rigtigt efter `SILENT_PAUSE_VISIBLE_MS` (20 sek.) mens appen er synlig, ellers efter `SILENT_PAUSE_MAX_MS` (30 min., Michaels valg 23-09-2026 — var 5 min.) fra pausen — `armSilentPauseTimer()` genberegnes ved hvert `visibilitychange`. `endSilentPause()` = rigtigt stop, `clearSilentPause()` = afslut uden at røre afspilningen (før reconnect). `stopPlayback()` (søvntimer, Sonos-cast) stopper altid rigtigt. `play`-lytteren og `visibilitychange`-reconcileren ignorerer stilhedsløkken, så UI ikke viser "spiller". Ingen tone (modsat den fjernede 18 Hz-keepalive) og intet dataforbrug; lidt strøm i højst 30 min. `onWaiting` ignoreres under lydløs pause (løkken viste ellers "Forbinder"). Pc uændret. **Bekræftet på iPhone 23-09-2026:** pause på låseskærm → vent → PLAY virker (TC-17-16), pause i appen → lås → PLAY virker (TC-17-17).

**AirPods/headset og lydløs pause (diagnose-log 23-09-2026, iPhone 17 Pro Max, iOS 27.0)**: Ved AirPods ud sender iOS `ms:pause` (MediaSession) → appen starter stilhedsløkken → **iOS stopper selv løkken ca. 1 sek. senere** (`play()` afvises med `AbortError`) — Apples regel om, at lyd ikke må fortsætte i højttaleren. Ved AirPods ind sender iOS **ingen** play-kommando (kun "Connected"-lyden) — der er ingen automatisk genstart; tryk på headsettets PLAY sender `ms:play`, og det virker med appen åben. Med låst skærm kommer `ms:play` frem, men streamen går i stå (`stalled`) — iOS har strupet netværket, fordi siden ikke har spillet lyd siden frakoblingen (samme grundårsag som BUG-15). **`devicechange` fyrer slet ikke på iOS** (ingen hændelser i loggen) — `App.tsx`'s frakoblings-handler og `pauseForDisconnect()` virker derfor kun på andre platforme. Åbent — se `BUGS.md` BUG-15 for næste skridt.

**MIDLERTIDIG diagnose-log (23-09-2026)**: `src/utils/debugLog.ts` (`dlog()`, kun iOS) skriver MediaSession-kommandoer, audio-events, `visibilitychange` og `devicechange` til Firestore `debugLogs/{deviceId}` (buffer, flush efter 1,5 sek.). Kald i `useRadioStore.ts` og `App.tsx`. Læs med `node read-debuglogs.mjs`; `node clean-debuglogs.mjs` sletter Playwrights testlogs (UA "iPhone OS 18_0"). **Fjernes helt** (fil, kald, begge scripts og `debugLogs`-samlingen), når AirPods-sagen er afsluttet.
...
**BUG-15 genåbnet 23-09-2026 — delvist løst:** lydløs pause med stilhedsløkke (commit `839ac8a`, muted-versionen `06accb1` virkede ikke) er bekræftet på iPhone for pause på låseskærm / i appen. **Åbent:** AirPods ud → ind med låst skærm (TC-17-18 🔴) — se `BUGS.md` BUG-15 "Status ved dagens afslutning" for næste skridt.
...
- `TEST-CASES.md` — fuld testspecifikation: **116 test cases** fordelt på 17 grupper
- `TEST-REPORT.md` — testrapport: **115/116 godkendt**, 1 fejlet (TC-17-18, AirPods med låst skærm — åben) (23-09-2026); 108/108 automatiserede grønne
- Kør: `npx playwright test` (kræver netværk til live-appen, 4 workers anbefales på Windows)

## Hjælpescripts (rod-mappen)
- `read-debuglogs.mjs` / `clean-debuglogs.mjs` — **MIDLERTIDIGE** (23-09-2026): læser / rydder Firestore `debugLogs` fra diagnose-loggen (se "Audio-arkitektur"). Slettes sammen med loggen
