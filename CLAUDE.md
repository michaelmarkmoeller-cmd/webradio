...
**BUG-15 genåbnet 23-09-2026:** lydløs pause på iOS deployet (stilhedsløkke, commit `839ac8a` — muted-versionen `06accb1` virkede ikke) — afventer Michaels iPhone-test (TC-17-16..18).
...
- `tests/tc-17b.spec.ts` — TC-17-05..15: lydløs pause på iOS (11 tests; iPhone-UA, `page.clock`, simuleret `visibilityState`, MediaSession-handlere kaldes direkte). `WEBRADIO_URL` virker også her — `/api/artwork` stubbes, da Vite-dev-serveren ikke kører Vercel-funktioner (ellers dækker en vite-error-overlay knapperne)
...
- `TEST-CASES.md` — fuld testspecifikation: **112 test cases** fordelt på 17 grupper
- `TEST-REPORT.md` — testrapport: **109/112 godkendt**, 3 manuelle iPhone-tests afventer (23-09-2026); 104/104 automatiserede grønne
