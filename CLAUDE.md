...
- **80s80s-, radio SAW-familien m.fl.**: ingen ICY-metadata på webappen — men det er en klient-side parser-inkompatibilitet (undici/Node `fetch` vs. legacy `ICY 200 OK`-statuslinje), ikke en blokering fra stationens side. Se "ICY stream-metadata" ovenfor
